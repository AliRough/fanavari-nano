...
export default function TestticketExpert() {
  ...
  const getarchiveticket = () => {
    Axios.get("/api/admin/archive_ticket_expert")
      .then(async (res) => {
  ...
  const getopenedticket = () => {
    Axios.get("/api/admin/opened_ticket_expert")
      .then(async (res) => {
        // console.log(res.data)
  ...
  const getunresolvedticket = () => {
    Axios.get("/api/admin/unresolved_ticket_expert")
      .then(async (res) => {
        // console.log(res.data)
  ...
  const getAllticket = () => {
    Axios.get("/api/admin/ticket_expert")
      .then(async (res) => {
        console.log(res.data);
  ...
  return (
    <div>
      ...
      <div className="flex item-center p-3">
        <select
          name="select"
          className="border-gray-300 rounded-xl  text-xm mx-1"
        ...
        <p className="p-3">
          ...
          {selectedItem === "all"
            ...
            : selectedItem === "unresolved"
            ? unresolvedticket.length
            : ""}
        </p>
      </div>
          ...
          <tbody>
            {selectedItem === "all" &&
              ...
              Allticket.map((item, index) => {
                if (Allticket)
                  return <ItemInTesttikets key={index} item={item} />;
              })}
            {selectedItem === "open" &&
              openedticket &&
              openedticket.map((item, index) => {
                if (openedticket)
                  return <ItemInTesttikets key={index} item={item} />;
              })}
            {selectedItem === "archive" &&
            ...
            {selectedItem === "unresolved" &&
              unresolvedticket.map((item, index) => {
                if (unresolvedticket)
                  return <ItemInTesttikets key={index} item={item} />;
              })}
          </tbody>
